...
FlexCAN.begin() - Enable the CAN to start actively participating on the CANbus.

FlexCAN.end() - Disable the CAN from participating on the CANbus.  Pins remain assigned to the alternate function CAN0.

FlexCAN.send(msg) - Transmit a frame of 1 to 8 bytes using the given identifier.

FlexCAN.recv(msg) - Read a frame into the buffer if available.  recv() will return 1 if a frame was copied into the callers buffer, or 0 if no frame is available.
